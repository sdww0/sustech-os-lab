...
pub fn clone_child_process(
    ...
    // Clone VM
    let new_memory_space = current.memory_space.duplicate_with_new_frames();
    ...
    let process = Process::raw_new_user_process(context, new_memory_space, &current.heap);
    let threads = process.threads();
    if flags.contains(CloneFlags::CLONE_CHILD_CLEARTID) {
        let main_thread = threads.get(0).unwrap();
    ...
    process.set_parent_process(&current);
