...
use log::debug;
use ostd::{
    mm::{CachePolicy, Frame, FrameAllocOptions, PageProperty, Vaddr, VmSpace, PAGE_SIZE},
    sync::SpinLock,
};
...
impl MemorySpace {
    ...
    pub fn map(&self, mut mapping: VmMapping) -> Vec<Frame> {
        ...
        for (_index, frame) in frames.iter().enumerate() {
            cursor_mut.map(
                frame.clone(),
                PageProperty::new(mapping.perms.clone(), CachePolicy::Writeback),
            );
        }
